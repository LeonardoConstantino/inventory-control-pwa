...
const useInventoryFilters = (items: Item[], searchTerm: string, sortBy: SortOption, categoryFilter: string) => {
...
, [items]);
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (deferredSearchTerm.trim()) {
      ...
      result = result.filter((item) => item.searchableText.includes(searchLower));
    }
    
    // Filtro por categoria
    if (deferredCategoryFilter && deferredCategoryFilter !== 'all') {
      result = result.filter((item) => item?.category && item.category.includes(deferredCategoryFilter));
    }
    ...
    return result;
  }, [processedItems, deferredSearchTerm, deferredSortBy, deferredCategoryFilter]);
